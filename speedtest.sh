...
geekbench4() {
	...
	[ -n "$GEEKBENCH_URL_CLAIM" ] && echo -e "$GEEKBENCH_URL_CLAIM" >geekbench4_claim.url 2>/dev/null
	echo "" | tee -a "$log"
	echo -e " Cooling down..."
	sleep 9
	echo -ne "\e[1A"
...
iotest() {
	...
	ior1=$( (
		dd if="$benchram"/zero of=$NULL bs=512K count="$sbcount"
		rm -f test
	) 2>&1 | awk -F, '{io=$NF} END { print io}')
